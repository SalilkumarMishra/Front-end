...
const ProductQuickView = ({ product, onClose, onAddToCart, onAddToWishlist }) => {
  ...
  const nextImage = () => setCurrentIndex((index) => (index + 1) % images.length);
  const prevImage = () => setCurrentIndex((index) => (index - 1 + images.length) % images.length);
  const filledStars = "*".repeat(Math.round(product.rating || 0));
  const emptyStars = "-".repeat(5 - Math.round(product.rating || 0));
      ...
      <div className="bg-white w-full max-w-5xl rounded-2xl shadow-xl p-6 relative max-h-[92vh] overflow-y-auto animate-scaleIn">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="pr-4">
            ...
            <div className="mt-4">
              <p className="text-3xl text-[#82D173] font-bold">Rs. {product.price}</p>
